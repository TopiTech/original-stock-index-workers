    ...
    if (url.pathname === "/api/snapshot" && request.method === "GET") {
      try {
        ...
        if (cacheRow && (now - cacheRow.cached_at < SNAPSHOT_CACHE_TTL)) {
          return json(JSON.parse(cacheRow.data));
        }
        ...
        if (!latest) {
          return json({ error: "No data available from Yahoo Finance" }, 502);
        }
        ...
        ).bind(JSON.stringify(responseData), now).run();
